...
float fuzzy_speed = 0.0;  //模糊算得的速度值？？？？？？？
/*
 *  @brief      舵机模糊控制
 *  @since      v1.0
 *  P
...
//    float Rule_speed[7]={2.5,2.65,2.68,2.7,2.72,2.75,2.78};
    float Rule_kp[7]={12.7,13,13.4,13.8,14.3,14.8,15.2};          //kp隶属函数中心值
    float Rule_kd[7]={500,524,554,584,620,656,689};                 //kp隶属函数中心值
...
//      float Rule_speed[7]={2.7,2.9,3.25,3.28,3.33,3.36,3.4};
  ...
  //ec-3-2-1 0 1 2 3  //e  
    ...
    fec_speed = 0.0;   //我觉得要调参 
    fuzzy_kp = 0.0;
    fuzzy_kd = 0.0;
    fuzzy_speed = 0.0;
    
/********************************************************* 
	以下是模糊算法的实现，是公式，相当于接口，直接用即可 
*********************************************************/  
      
    /*-------------------------------------------*/
    /*---------------确定kp隶属度----不用调参------------*/
    /*-------------------------------------------*/
    if(fe < eRule[0]) //不在规定的范围内，限幅    		       
    {
        eFuzzy[0] =1.0; 
        pe= 0;         
    }
