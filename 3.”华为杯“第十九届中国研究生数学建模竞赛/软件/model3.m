...
if size(sj1,1)>4
if che(tag,2)==2
    if che(size(sj1,1),2)==1&&che(size(sj1,1)-1,2)==1&&che(size(sj1,1)-2,2)==2
        order1=0;
    end
    order1=1;
else
...
if che(size(sj1,1),3)~=che(tag,3)
    order2=1;
            ...
            if(tag<=size(txt,1))
                if((order1&&order2)&&tutu<size(che,1)-10)%此处条件可以改为对送车横移机如何选择进入反车道
                    ran2=rand<0.5;
                    if(ran2&&fan_shijian(1,1)<=0 && fan_car(1,1)==0)
                        flag2=1;
                        fan_t=fan_t+1;
...
%             end
            ...
            if (jieche(t+2)<=0)
                %判断是否返回道有车
                ran1 = rand < 0.5;
                if(ran1 &&(flag1~=1) && (fan_shijian(1,10)<=0 )&& (fan_car(1,10)>0))
                    flag1=1;%1送返回道来的车
                    %判断送入哪个车道
