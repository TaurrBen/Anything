...
class BayesianCNN(PyroModule):
    ...
    def model(self,X,y):
        ...
        # sigma = pyro.sample("sigma", dist.Uniform(0., 1.))+1e-04
        sigma = 1
        with pyro.plate("data", X.shape[0]):
            pyro.sample("obs", dist.Normal(mu*torch.ones_like(mu), sigma*torch.ones_like(mu)).to_event(2), obs=y)
...
def train(train_loader, test_loader,unshuffle_train_loader,finaltest_loader,num_iter):
    model = BayesianCNN()
    loss_func = RMSELoss()
    train_loss_epoch = []
    test_loss_epoch = []
    ...
    loss = pyro.infer.Trace_ELBO()
    svi = pyro.infer.SVI(model.model, model.guide, optim, loss=loss)
    pyro.clear_param_store()
    print("\tIter | \tTrain Loss | \tTest Loss")
    for iter in range(num_iter):
        ...
        for i, (data_tr, label_tr) in enumerate(tqdm(train_loader)):
            batch_counter_tr += 1
            output_tr,_ = predict(model.model,model.guide,data_tr.float())
            loss_tr = svi.step(data_tr,label_tr)
            running_loss_tr += loss_tr
            if iter == num_iter - 1:
                train_output += output_tr.flatten().tolist()
        ...
        if iter == num_iter - 1:
            if unshuffle_train_loader is not None:
                train_output = []
                for i, (data_uns_tr, lable_un_tr) in enumerate(tqdm(unshuffle_train_loader)):
                    output_uns_tr,_ = predict(model.model,model.guide,data_uns_tr.float())
                    train_output += output_uns_tr.flatten().tolist()
        model.eval()
        for i, (data_te, label_te) in enumerate(tqdm(test_loader)):
            batch_counter_te += 1
            output_te,_ = predict(model.model,model.guide,data_te.float())
    ...
    model.eval()
    for i, (data_fte) in enumerate(tqdm(finaltest_loader)):
        output_fte,_ = predict(model.model,model.guide,data_fte.float())
        finaltest_output += output_fte.flatten().tolist()

    return train_loss_epoch, test_loss_epoch, train_output, test_output, finaltest_output

def predict(model,guide,X,num_sample=20):
    return 0, 0
