...
class LSTM(torch.nn.Module):
    def __init__(self):
        ...
        self.lstm = torch.nn.LSTM(input_size=32, hidden_size=64, num_layers=1, batch_first=True, dropout=0.1, bidirectional=True)
        self.linearSeq = torch.nn.Sequential(torch.nn.Linear(64*2,64),torch.nn.ReLU(),torch.nn.Linear(64,1))
...
def train(model,guide,train_loader, test_loader,unshuffle_train_loader,finaltest_loader,num_iter):
    loss_func = RMSELoss()
    train_loss_epoch = []
    test_loss_epoch = []
    train_output = []
    ...
    train_output_raw = []
    test_output_raw = []
    finaltest_output_raw = []
    optim = pyro.optim.Adam({"lr": 0.01,"betas":(0.9, 0.999),"eps":1e-04,"weight_decay": 0})
    loss = pyro.infer.Trace_ELBO()
    ...
    for iter in range(num_iter):
        ...
        for i, (data_tr, label_tr) in enumerate(tqdm(train_loader)):
            batch_counter_tr += 1
            output_tr,output_tr_std,output_tr_raw = predict(guide,data_tr.float())
            loss_tr = svi.step(data_tr,label_tr)
            running_loss_tr += loss_tr
            if iter == num_iter - 1:
                train_output += output_tr.flatten().tolist()
                train_output_std += output_tr_std.flatten().tolist()
                output_tr_raw_stack = torch.stack(output_tr_raw)
                train_output_raw += output_tr_raw_stack.transpose(0, 1)
        epoch_loss_tr = running_loss_tr / batch_counter_tr
        train_loss_epoch.append(epoch_loss_tr)
        ...
        if iter == num_iter - 1:
            if unshuffle_train_loader is not None:
                ...
                train_output_raw = []
                for i, (data_uns_tr, lable_un_tr) in enumerate(tqdm(unshuffle_train_loader)):
                    output_uns_tr,output_uns_tr_std,output_uns_tr_raw = predict(guide,data_uns_tr.float())
                    train_output += output_uns_tr.flatten().tolist()
                    train_output_std += output_uns_tr_std.flatten().tolist()
                    output_uns_tr_raw_stack = torch.stack(output_uns_tr_raw)
                    train_output_raw += output_uns_tr_raw_stack.transpose(0, 1)
        # model.eval()
        for i, (data_te, label_te) in enumerate(tqdm(test_loader)):
            batch_counter_te += 1
            output_te,output_te_std,output_te_raw = predict(guide,data_te.float())
            # loss_te = loss_func(torch.mean(torch.stack(output_te), 0), label_te)
            loss_te = loss_func(output_te, label_te)
            running_loss_te += loss_te.item()
            if iter == num_iter- 1:
                test_output += output_te.flatten().tolist()
                test_output_std += output_te_std.flatten().tolist()
                output_te_raw_stack = torch.stack(output_te_raw)
                test_output_raw += output_te_raw_stack.transpose(0, 1)
    ...
    # model.eval()
    for i, (data_fte) in enumerate(tqdm(finaltest_loader)):
        output_fte,output_fte_std,output_fte_raw = predict(guide,data_fte.float())
        finaltest_output += output_fte.flatten().tolist()
        finaltest_output_std += output_fte_std.flatten().tolist()
        output_fte_raw_stack = torch.stack(output_fte_raw)
        finaltest_output_raw += output_fte_raw_stack.transpose(0, 1)

    return train_loss_epoch, test_loss_epoch, (train_output,train_output_std,train_output_raw), (test_output,test_output_std,test_output_raw), (finaltest_output,finaltest_output_std,finaltest_output_raw), (model,guide)

def predict(guide,X,num_sample=20):
