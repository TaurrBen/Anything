...
def load_dataset(read_path, sub_dataset, max_life, train, test, finaltest):
...
class train_dataset(Dataset):
    def __init__(self, read_path, sub_dataset, window_size, max_life, scaler, scaler_range):
    ...
    def to_preprocess(self):
        ...
        training_set = load_dataset(self.read_path, self.sub_dataset, self.max_life, train=True, test=False, finaltest=False)
...
class test_dataset(Dataset):
    ...
    def to_preprocess(self):
        ...
        testing_set = load_dataset(self.read_path, self.sub_dataset, self.max_life, train=False, test=True, finaltest=False)
...
class finaltest_dataset(Dataset):
    ...
    def to_preprocess(self):
        ...
        final_testing_set = load_dataset(self.read_path, self.sub_dataset, self.max_life, train=False, test=False, finaltest=True)
...
def LoadCMAPSS(read_path, sub_dataset, window_size, max_life, scaler, scaler_range, shuffle, batch_size):
    
    if shuffle:

        tr_dataset = train_dataset(read_path, sub_dataset, window_size, max_life, scaler, scaler_range)
        print('\t Length of shuffeled train data: ' , len(tr_dataset))
        train_loader = DataLoader(tr_dataset, batch_size, shuffle=True,drop_last=True)
    ...
    te_dataset = test_dataset(read_path, sub_dataset,  window_size, max_life, scaler, scaler_range)
    print('\t Length of test data: ', len(te_dataset))
    test_loader = DataLoader(te_dataset, batch_size, shuffle=False)

    finalte_dataset = finaltest_dataset(read_path, sub_dataset,  window_size, max_life, scaler, scaler_range)
    print('\t Length of finaltest data: ' , len(finalte_dataset))
    finaltest_loader = DataLoader(finalte_dataset, batch_size, shuffle=False)

    return train_loader, test_loader, unshuffle_train_loader, finaltest_loader
