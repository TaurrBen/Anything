...
def train_actual_predicted(read_path, sub_dataset, window_size, max_life, fusion, train_output, alpha_grid, alpha_low, alpha_high, _COLORS):
...
def test_actual_predicted(read_path, sub_dataset, window_size, max_life, fusion, test_output, alpha_grid, alpha_low, alpha_high, _COLORS):
...
def fianltest_actual_vs_predicted(read_path, sub_dataset, window_size, max_life, fusion, finaltest_output, alpha_grid,
    ...
    for engine_id, data in finaltest_rul_group:
        if engine_id in plot_ids:
            ...
            predicted_rul_std = predicted_rul_std.set_index('cycle')
            predicted_rul_std.columns = ['Predicted RUL']
            upper_rul = predicted_rul + 3*predicted_rul_std
            lower_rul = predicted_rul - 3*predicted_rul_std
            ...
            plt.plot(predicted_rul, color=_COLORS[0], alpha=alpha_high, linestyle="solid", linewidth=4, label='Predicted RUL')
            plt.plot(actual_rul, color=_COLORS[1], alpha=alpha_high, linestyle="dashed", linewidth=4, label='Actual RUL')
            plt.fill_between(data['cycle'],y1=lower_rul['Predicted RUL'].tolist(), y2=upper_rul['Predicted RUL'].tolist(), color=_COLORS[2], alpha=alpha_high, label='3-sigma confidence')
