...
class LSTM(torch.nn.Module):
    def __init__(self,batch_size,seq_length,feature_length,hidden_size=100,num_layers=3,bias=True,batch_first=True,num_directions=1):
        ...
        self.lstm1 = torch.nn.LSTM(self.feature_length, hidden_size=self.hidden_size,num_layers=self.num_layers,bias=self.bias,batch_first=self.batch_first)
        self.fc1 = torch.nn.Linear(self.hidden_size, 20)
        self.fc2 = torch.nn.Linear(20, 1)
        self.relu = torch.nn.ReLU()

    def forward(self,input_):
        lstm1_h0 = torch.randn(self.num_layers * self.num_directions, self.batch_size, self.hidden_size)
        lstm1_c0 = torch.randn(self.num_layers * self.num_directions, self.batch_size, self.hidden_size)

        out = input_.view(input_.size(0),input_.size(2),input_.size(3))
        import torch.nn.utils.rnn as rnn_utils
        length = torch.tensor([self.batch_size,self.seq_length,self.feature_length])
        out = rnn_utils.pack_padded_sequence(out, length,batch_first=True)
        out,_ = self.lstm1(out,(lstm1_h0,lstm1_c0))
        ...
        for name, p in self.net.named_parameters():
            ...
            if name.endswith("weight"):
                ...
                if name.split('.')[1].startswith("weight"):
                    priors[name] = dist.Normal(torch.zeros_like(p), torch.ones_like(p)).to_event(2)
                elif name.split('.')[1].startswith("bias"):
                    priors[name] = dist.Normal(torch.zeros_like(p), torch.ones_like(p)).to_event(1)
