...
def train_actual_predicted(read_path, sub_dataset, window_size, max_life, fusion, uncertainty, train_output, alpha_grid,
                           alpha_low, alpha_high, _COLORS):
    train_data = CMAPSS_Dataloader.load_dataset(read_path, sub_dataset, max_life, fusion, train=True, test=False,
                                                finaltest=False)
    ...
    if uncertainty:
        train_data['Predicted RUL Epistemic Uncertainty'] = train_output[2]
        train_data['Predicted RUL Aleatoric Uncertainty'] = train_output[3]
    ...
    for engine_id, data in train_rul_group:
        ...
        factor = 1
        upper_rul = predicted_rul + factor * predicted_rul_std
        lower_rul = predicted_rul - factor * predicted_rul_std
...
def test_actual_predicted(read_path, sub_dataset, window_size, max_life, fusion, uncertainty, test_output, alpha_grid,
                          alpha_low, alpha_high, _COLORS):
    test_data = CMAPSS_Dataloader.load_dataset(read_path, sub_dataset, max_life, fusion, train=False, test=True,
                                               finaltest=False)
    ...
    if uncertainty:
        test_data['Predicted RUL Epistemic Uncertainty'] = test_output[2]
        test_data['Predicted RUL Aleatoric Uncertainty'] = test_output[3]
...
def fianltest_actual_vs_predicted(read_path, sub_dataset, window_size, max_life, fusion, uncertainty, finaltest_output,
                                  alpha_grid,
                                  alpha_low, alpha_high, _COLORS, finalte_dataset):
    finaltest_data = CMAPSS_Dataloader.load_dataset(read_path, sub_dataset, max_life, fusion, train=False, test=False,
                                                    finaltest=True)
    ...
    if uncertainty:
        finaltest_data['Predicted RUL Epistemic Uncertainty'] = finaltest_output[2]
        finaltest_data['Predicted RUL Aleatoric Uncertainty'] = finaltest_output[3]
        finaltest_data['Predicted RUL Total Uncertainty'] = finaltest_data['Predicted RUL Epistemic Uncertainty'] + \
                                                            finaltest_data['Predicted RUL Aleatoric Uncertainty']
