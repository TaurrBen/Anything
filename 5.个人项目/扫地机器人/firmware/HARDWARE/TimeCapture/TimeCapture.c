...
{
    TIM8_Capture_Init();//风机编码结构体
    TIM1_Capture_Init();//左右轮编码结构体
}
/*
...
{
    TIM8_CH4_FanCaptureInit(TIM8_ARR,TIM8_PSC);//50000，72，周期50ms
}
/*
...
{
    GPIO_InitTypeDef  GPIO_InitStructure;          //GPIO结构体，设置引脚、速度、模式
    TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;//定时器结构体，设置psc,arr,countmode，时钟分频
    NVIC_InitTypeDef NVIC_InitStructure;           //中断设置结构体，中断通道、抢占优先级、子优先级、使能等
...
void TIM1_CH12WheelCaptureInit(u16 arr,u16 psc)
{
    ...
    TIM_ICInitStructure.TIM_ICSelection = TIM_ICSelection_DirectTI; 
    TIM_ICInitStructure.TIM_ICPrescaler = TIM_ICPSC_DIV1;			//多少个边沿触发一次捕获 
    TIM_ICInitStructure.TIM_ICFilter = 0x05;  //捕获到上升沿后，多少个采样周期内依然是高电平才认为是成功捕获             
...
{
    if(SET == TIM_GetITStatus(TIM1,TIM_IT_CC1) )     //检测到捕获中断
    {
        TIM_ClearITPendingBit( TIM1,TIM_IT_CC1 );    //清除中断标志位    
        g_tLeftWheel.TimeOut=0;
        g_tLeftWheel.usNew = TIM_GetCapture1( TIM1 );//编码器捕获值  
        ...
        GET_LEFT_ENCODE_DONE(); //更新历史捕获值，数据缓冲区下标加1
    }
